...
const ChatWithGemini = () => {
    ...
    const handleclick=(e)=>{
    updateMessage([...messages, { "role": "user", "parts": [{ "text": e.target.textContent }] }])
    sendMessages({ message: e.target.textContent, history: messages })
    ...
    return (
        <>
            <Box className="w-[100%] self-center max-w-[1400px] m-4 overflow-auto rounded-md h-[80%] items-center">
             ...
             <div>
                <button className="btn" onClick={handleclick}>👨‍🎓 Guide me about studies</button>
                <button className="btn" onClick={handleclick}>🛣️RoadMap to get Success in Life</button>
                <button className="btn" onClick={handleclick}>🖊️Provide best carrier option</button>
             </div>
